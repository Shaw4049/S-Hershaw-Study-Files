# Creating an empty list of gamers
gamers = []

# Create a function called add_gamer that takes two parameters: gamer and gamers_list. The function should check that the 
# argument passed to the gamer parameter has both "name" and a "availability" as keys and if so add gamer to gamers_list.
...
# Adding the first gamer to the list.
kimberly = { 
    'name':"kimnerly", 
    'availability': ["Monday", "Tuesday", "Friday"] }
add_gamer(kimberly, gamers)
# Confirming details added correctly.
print(gamers)
...
# Create a function called build_daily_frequency_table that takes no argument returns a dictionary with the days of 
# the week as keys and 0s for values. We'll be using this to count the availability per night. Call build_daily_frequency_table 
# and save the results to a variable called count_availability.
def build_daily_frequency_table():
    frequency_table_dict = {'Monday':0, 'Tuesday':0, 'Wednesday':0, 'Thursday':0, 'Friday':0, 'Saturday':0, 'Sunday':0 }
    return frequency_table_dict
count_availability = build_daily_frequency_table()
print(count_availability)

# Write a function called calculate_availability that takes a list of gamers as an argument gamers_list and a frequency table 
# available_frequency. The function should iterate through each gamer in gamers_list and iterate through each day in the gamer's availability. 
